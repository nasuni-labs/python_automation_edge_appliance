    ...
    while True:
        ...
        # Starting screen
            ...
            if connection_requests == 0:
                logging.warning("%s: We haven't started the AD wizard. Waiting 10 seconds", hostname)
                resubmits += 1
                if resubmits > 12:
                   logging.warning("%s: We've been waiting for the AD wizard to start for 2 minutes. Something might be wrong. Please check the appliance.", hostname)
                else:
                    time.sleep(10)
                    continue
            ...
            try:
                ...
                text = page.locator("#ds-auth-confirm").text_content()
                # text = text.partition("Enter the user name and password of a directory user")[0]
                logging.warning("%s: Domain join error bar: %s", hostname, text)
                logging.warning("%s: Total connection requests: %d", hostname, connection_requests)
            except PlaywrightTimeout:
                logging.error("%s: Could not find domain join error bar.", hostname)
            return {
                "success": False,
                "errors": {
