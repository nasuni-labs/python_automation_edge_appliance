...
if __name__ == "__main__":
    with open("vars.json", "r") as f:
        ...
        query = ""
        try:
            query = auth_to_api(merged_vars["nmc_ip"], merged_vars["api_user"], merged_vars["api_pass"], merged_vars["validate_ssl"])
            token = query["token"]
        except KeyError as e:
            logging.error("Failed to authenticate to the API. Please see the below error.")
            logging.error(query)
            exit(1)
        merged_vars["api_token"] = token
        # print(merged_vars["api_token"])
