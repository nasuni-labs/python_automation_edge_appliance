...
def grab_machine_status(page, nmc_ip, serial_number):
    t = 0
    while True:
        if t > 15:
            logging.error("Too many throttles. Exiting")
            return {
...
class EdgeAppliance:
    ...
    def get_state_from_nmc(self):
        self.status["gui_data"] = grab_machine_status(self.page, self.vars_dict["nmc_ip"], self.serial_num)
    ...
    def get_login(self):
        ...
        if self.make_login_attempt(login, password):
            ...
            return 0
        logging.warning("Used NMC cred %s and %s and couldn't login. We will try the Admin credentials", login, password)
        login = self.vars_dict["edge_user"]
        password = self.vars_dict["edge_pass"]
        if self.make_login_attempt(login, password):
            logging.warning("Logged in under Admin creds")
            return 0
        logging.warning("Login shows a failure with both filer-based logons and NMC based logons. We will retry")
        return 1
...
def edge_run_wrapper(page, ip_addy, data_block):
    ...
    try:
        version = edge.status["gui_data"]["global_status"]["version"]
    except KeyError:
        version = "unknown"
